...
const encodeDir = async () => {
    ...
    console.log('from worker: nikonikonii it works')
    try {
            ...
            files.forEach((filename) => {
                ...
                ffmpeg()
                    ...
                    .on("error", (err, stdout, stderr) => {
                        ...
                        execFile(cwebp, [fullPath, '-o', outputPath], (err) => {
                            if (err) {
                                console.error(`(cwebp fallback) An Error ocurred trying to encode image: ${err}`)
                            }
                            let endTime = Date.now()
                            console.log(
                                `(cwebp fallback) encoding finished after ${(endTime - startTime) / 1000} seconds`)
                            
                            filevars.setFileLength(filevars.getFileLength() - 1)
                            getArchive().append(fse.createReadStream(outputPath), {name: filename})                    
                })
